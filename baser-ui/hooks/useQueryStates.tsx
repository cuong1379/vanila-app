...
export type SetValues<T> = React.Dispatch<React.SetStateAction<Partial<Values<T>>>>
...
export function useQueryStates<T extends object>(
  ...
  const getValues = React.useCallback((): Values<T> => {
    if (typeof window === 'undefined') {
      ...
      return Object.keys(keys).reduce((obj, key) => ({ ...obj, [key]: null }), {} as Values<T>)
    }
  ...
  const update = React.useCallback(
    (stateUpdater: React.SetStateAction<Partial<Values<T>>>) => {
      const isUpdaterFunction = (input: any): input is (prevState: Partial<Values<T>>) => Partial<Values<T>> => {
        return typeof input === 'function'
      }

      // Resolve the new values based on old values & updater
      const oldValues = getValues()
      const newValues = isUpdaterFunction(stateUpdater) ? stateUpdater(oldValues) : stateUpdater
